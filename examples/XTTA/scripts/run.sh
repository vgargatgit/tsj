...
run_test() {
  ...
  # Add interop flags for interop tests
  if [ "$category" = "interop" ]; then
    args="$args --jar $DEPS_DIR/xtta-complex.jar --interop-policy broad --ack-interop-risk"
  fi
  ...
  # Check for false results in stdout
  local stdout_lines
...
echo "XTTA — eXtreme TypeScript Torture App"
echo "======================================"
echo
...
echo "--- Built-in Method Tests ---"
for f in "$ROOT_DIR"/src/builtins/*.ts; do
  [ -f "$f" ] && run_test "$f" "builtins"
done
echo

echo "--- Java Interop Tests ---"
if [ -f "$DEPS_DIR/xtta-complex.jar" ]; then
